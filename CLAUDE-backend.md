# Tbilisi Services — Backend

Backend локального маркетплейса услуг для Тбилиси (Telegram Mini App).
MVP-стадия: проверяем спрос на реальных пользователях, набранных через
локальные Telegram/Facebook-группы.

## Стек

- Node.js + TypeScript + Express
- PostgreSQL через Prisma ORM
- Деплой: Docker (см. `Dockerfile`) через Coolify, self-hosted на VPS
- Без фреймворков для админки — обычный HTML/CSS/JS в `public/admin`

## Архитектура — строго модульная

```
src/modules/<name>/
  router.ts   — маршруты (публичные + admin-роутер отдельно)
  service.ts  — запросы к БД через Prisma
```

Модули: `categories`, `services`, `providers`, `requests`, `offers`,
`reviews`, `messages` (чат по заказу), `auth` (Telegram initData),
`admin` (аналитика).

Исключение — `src/bot/bot.ts`: Telegram-бот на Telegraf, канал уведомлений,
без переписки — все ответы короткие и ведут в Mini App кнопкой
(`MINI_APP_URL`). Не HTTP-модуль (нет router.ts/service.ts, не монтируется
в `app.ts`), запускается отдельно в `src/index.ts` через `startBot()`. Без
`TELEGRAM_BOT_TOKEN` просто не стартует (WARN в логах), остальной backend
продолжает работать.

`notifyUser(telegramId, text)` в `bot.ts` — уведомления клиенту/мастеру
лично (в отличие от `notifyAdmin`, который всегда шлёт в один и тот же
`SUPPORT_ADMIN_CHAT_ID`). Вызывается из сервисов, не await'ится (сбой
доставки не должен ронять запрос, тот же паттерн, что и у `notifyAdmin` в
разделе «Поддержка» ниже):
- `requestsService.create` → всем мастерам с подходящей услугой
  (`Provider.notifyRequests`) — новая заявка.
- `offersService.create` → клиенту, автору заявки (`User.notifyOrders`) —
  новый отклик.
- `offersService.respond` (accepted) → мастеру, чей отклик приняли
  (`Provider.notifyOrders`) — заказ подтверждён.
- `reviewsService.create` → мастеру (`Provider.notifyReviews`) — новый
  отзыв от клиента.
- `clientReviewsService.create` → клиенту (`User.notifyReviews`) — новый
  отзыв от мастера.

Все четыре флага (`User.notifyOrders/notifyReviews`,
`Provider.notifyRequests/notifyReviews/notifyOrders`) по умолчанию `true`,
меняются самим пользователем: `PUT /api/users/prefs` (клиент) и
`PUT /api/providers/:id/prefs` (мастер, с проверкой `telegramId` — см.
`providersService.updatePrefs`, аналогично `deactivate`). Чат
(`notifyChat` на фронте) — не реализован, эти тумблеры пока чисто
локальные, чат-фичи в API/схеме нет.

Правило: **новая функциональность = новый модуль** той же структуры.
Не сваливать логику в `app.ts` — там только подключение роутеров.
Публичные роуты монтируются на `/api/<module>`, админские — на
`/api/admin/<module>` и защищены `adminAuth` (Basic Auth,
логин/пароль из env `ADMIN_USERNAME` / `ADMIN_PASSWORD`).

## Модель данных

`prisma/schema.prisma` — источник истины по структуре БД. Ключевые сущности:
`User` (роль client/provider/admin, привязан к `telegramId`, свои
`rating`/`reviewCount`), `Provider` (рейтинг, цена, район, услуги —
many-to-many через `ProviderService` / `ProviderArea`), `Category` →
`Service`, `Request` (заявка клиента), `Offer` (отклик мастера на
заявку), `Review` (отзыв клиента о мастере), `ClientReview` (зеркало
`Review` в обратную сторону — отзыв мастера о клиенте).

Алгоритм подбора мастеров — `src/modules/requests/matching.ts`, веса:
услуга 30% / расстояние 20% / цена 15% / рейтинг 15% / отзывы 10% /
подтверждён профиль 10%. Не менять веса без явного запроса — они
зафиксированы по исходному продуктовому документу. Последний фактор был
«скорость ответа» (`responseTimeMin`) — заменён на `Provider.verified`,
т.к. `responseTimeMin` был статическим полем по умолчанию 30 мин у всех
мастеров и никогда не пересчитывался по реальному поведению (по факту не
измерял ничего). `verified` подтверждается только вручную админом —
пока нет реальной метрики отзывчивости мастера, честный сигнал доверия.

## Роли пользователей (важно)

Приложение различает клиента и мастера **на фронтенде**, не здесь — но
backend даёт для этого отдельные публичные точки входа:
- `POST /api/providers/register` — самостоятельная регистрация мастера
  из Mini App (без админки), без подтверждения (`verified: false` по
  умолчанию, админ подтверждает вручную позже).
- `GET /api/providers/by-telegram/:telegramId` — фронтенд проверяет,
  зарегистрирован ли уже открывший приложение как мастер.
- `GET /api/requests/open?serviceId=` — лента открытых заявок для мастера
  (мастер может оказывать несколько услуг — `serviceId` принимает список
  через запятую).
- `GET /api/requests/mine?telegramId=` — заявки клиента + отклики по ним.

## Админка: вкладка «Пользователи»

Одна вкладка на всех — не «Мастера» и «Клиенты» отдельно, потому что это
один и тот же `User` (`Provider` — просто опциональная связанная запись).
Список — `GET /api/admin/users` (без фильтра по `role`, отдаёт всех, с
вложенным `provider.services/areas` для бейджей и фильтров), детальная
карточка — `GET /api/admin/users/:id` (клиентская история + вложенный
`provider` целиком, если он есть — одним запросом на обе вкладки
детальной карточки). Фильтры (услуга/район/рейтинг/статус) и поиск —
целиком на фронте (`public/admin/admin.js`, `applyUserFilters`), без
query-параметров на backend — датасет для MVP небольшой, а серверные
фильтры добавили бы отдельный слой параметров ради не очень большой
экономии. «Добавить мастера» на этой же вкладке — переиспользует `POST
/api/admin/providers` (раньше была отдельная вкладка «Мастера», её
убрали, чтобы не было двух разных экранов управления мастером с разным
набором возможностей).

## Удаление мастера/клиента (админка)

`DELETE /api/admin/providers/:id` и `DELETE /api/admin/users/:id` удаляют
каскадом всё связанное — услуги/районы мастера, его отклики и отзывы (в
обе стороны — `Review` и `ClientReview`); у клиента — его заявки и
отзывы в обе стороны (`onDelete: Cascade` в `schema.prisma`, модели
`Provider`, `ProviderService`, `ProviderArea`, `Offer`, `Request`,
`Review`, `ClientReview`). Удаление мастера не удаляет его `User` —
роль откатывается на `client`, чтобы человек мог зарегистрироваться
заново.

Это единственное место, где данные удаляются по-настоящему — только
руками админа. Самостоятельное удаление аккаунта из Mini App (см. ниже)
никогда не делает hard delete.

## Блокировка админом (модерация, не путать с деактивацией)

`User.blocked` (клиент) и `Provider.blocked` (мастер, было раньше) —
ручная блокировка нарушителей из админки. Принципиально другое от
`active`/деактивации выше: `blocked` ставит только админ, ограничивает
только **новые** действия, вся история (заявки/отклики/отзывы/чаты)
остаётся видна и доступна как есть — профиль просто перестаёт быть
активным участником рынка:
- `requestsService.create` — заблокированный клиент не может создать
  новую заявку (403).
- `offersService.create` — заблокированный мастер (`provider.blocked`
  **или** `provider.user.blocked` — блокировка на уровне `User`
  перекрывает и мастерскую сторону тоже) не может откликнуться (403).
- `providersService.create` при самостоятельной регистрации (`POST
  /api/providers/register`, через `assertNotBlocked`) — заблокированный
  клиент не может обойти блокировку, зарегистрировавшись мастером.
  Ручное добавление мастера админом (`POST /api/admin/providers`, тот же
  `providersService.create`) эту проверку не проходит — админ добавляет
  сам, ему видно кого.
- Отзывы, чат, поддержка — **не** гейтятся блокировкой (осознанно, см.
  обсуждение фичи): не входит в текущий охват, при необходимости
  расширить теми же двумя строчками (`if (x.blocked) throw...`) в
  соответствующем `service.ts`.
- Ставится/снимается через `PUT /api/admin/users/:id` и `PUT
  /api/admin/providers/:id` (тело `{ blocked: true|false }`), в админке —
  кнопка «Заблокировать»/«Разблокировать» на вкладке «Пользователи»
  (см. ниже).

## Самостоятельное удаление аккаунта — «мягкая» деактивация

Кнопка «Удалить аккаунт» в Mini App (у клиента и у мастера) не должна
обнулять рейтинг, если человек передумает и «зарегистрируется» заново —
у него тот же `telegramId`, значит тот же `User`/`Provider`. Поэтому это
не `DELETE`, а флаг `active: false` на `User` (клиент) или `Provider`
(мастер) — профиль и вся история остаются в базе, но скрываются из
выдачи:
- `Provider.active` (отдельно от `blocked` — то модерация админом, это
  самостоятельное отключение) — фильтруется в `providersService.list`,
  `getByTelegramId`, `requestsService.matchCandidates`.
- `User.active` — фильтруется в `requestsService.listOpen` (заявки
  деактивированного клиента не показываются мастерам).
- `PUT /api/users/deactivate` (body: `telegramId`) и
  `PUT /api/providers/:id/deactivate` (body: `telegramId`, сверяется с
  `provider.user.telegramId` — как в `requestsService.archive`) — обе
  ручки заодно сбрасывают `User.entryRole` в `null`, чтобы при следующем
  открытии показался экран выбора роли, а не автовход в уже «удалённый»
  кабинет (см. `useEffect` в начале `App.jsx` на фронтенде).
- **Реактивация без отдельной кнопки** — происходит прозрачно в
  `usersService.setPrefs`, когда пользователь в следующий раз выбирает
  роль (`entryRole` становится `"client"`/`"provider"`): для клиента там
  же выставляется `User.active: true`, для мастера — ещё и
  `Provider.active: true` через `updateMany` по `userId`. Осознанно не
  делается в `GET`-ручках (`getByTelegramId` и т.п.) — у чтения не
  должно быть побочных эффектов.

## Рейтинг клиента (симметрично рейтингу мастера)

Как и мастер, клиент имеет `rating`/`reviewCount` на `User`, посчитанные
по отзывам от другой стороны — только в обратную сторону: мастер
оставляет отзыв о клиенте после завершения заказа. Модуль
`src/modules/clientReviews` — зеркало `src/modules/reviews`:
- `POST /api/client-reviews` — мастер оставляет отзыв о клиенте
  (`providerId`, `requestId`, `userId` клиента, `rating`, `text?`).
  В отличие от `reviewsService.create` (которому нужен upsert по
  `telegramId`, т.к. это может быть первый визит клиента), здесь upsert
  не нужен — и `Provider`, и `User` к этому моменту уже точно
  существуют. Пересчитывает `User.rating`/`reviewCount` в той же
  транзакции.
- `GET /api/users/by-telegram/:telegramId` / `GET /api/users/:id` —
  публичный профиль клиента (рейтинг + отзывы от мастеров), симметрично
  `providersRouter`'s `by-telegram`/`:id`. Отдельно от `usersAdminRouter`
  (закрыт `adminAuth`, для полной карточки клиента в админке).
- `GET /api/admin/client-reviews`, `DELETE /api/admin/client-reviews/:id`
  — модерация, зеркало `reviewsAdminRouter`.

Заказ считается завершённым по `Request.status === "completed"` (ставит
клиент через `POST /api/reviews`, см. `reviewsService.create`) — это
единственное условие, открывающее мастеру возможность оставить отзыв о
клиенте; отдельного «мастер подтвердил завершение» шага нет.

## Монетизация — гибрид «подписка как скидка на лиды»

Модуль `src/modules/subscriptions/`. Модель: у мастера есть `balance` (целые
лари) и `tier` (`free`/`pro`). Отклик на заявку списывает деньги с баланса;
Free платит полную цену за лид, Pro — дешевле, плюс буст в подборе и ранний
доступ к заявкам. Вся тарифная сетка — в одном месте,
`src/modules/subscriptions/pricing.ts`, больше нигде цены не хардкодить.

**Сейчас все цены в `pricing.ts` — нули.** Это рабочий режим, а не заглушка:
механика полностью работает (списания создаются, `LeadCharge` копится), но
суммы нулевые — приложение внешне бесплатное, деньги никто не платит.
Включение монетизации в будущем — правка чисел в `pricing.ts`, без изменения
кода. Интеграции с реальной оплатой (в т.ч. Telegram Stars) в проекте нет —
пополнение баланса и выдача Pro только вручную через админку
(`POST /api/admin/subscriptions/topup`, `POST /api/admin/subscriptions/pro`).

Деньги — только целые числа (`Int` в схеме), никаких `Float`/`Decimal`:
баланс `120` = 120 ₾, не копейки/тетри.

`subscriptionsService.getEffectiveTier(provider)` — единственный источник
истины о тарифе (pro, только если `tier === "pro"` И `tierUntil` в будущем).
Читать `provider.tier` напрямую нигде нельзя — иначе истёкшая подписка
продолжит работать. Это же правило соблюдается в клиентском JS админки
(`isEffectivelyPro`/`tierLabel` в `admin.js`).

`chargeLead(providerId, requestId, tx?)` — идемпотентно (уникальный индекс
`LeadCharge @@unique([providerId, requestId])`, повторный вызов на ту же
пару ничего не списывает), и атомарно (баланс уменьшается условным
`updateMany` с проверкой `balance >= price` внутри, а не отдельными
проверкой-и-записью — иначе два почти одновременных отклика могли бы увести
баланс в минус). Принимает опциональный Prisma-`tx`, чтобы вызывающий код
мог включить списание в свою транзакцию — так и сделано в
`offers/service.ts`: списание и создание отклика — одна транзакция.

Три точки интеграции с существующей логикой:
- `offers/service.ts` → `create()` — списание перед созданием отклика.
- `requests/matching.ts` / `requestsService.matchCandidates` — итоговый
  `overall` умножается на `matchBoost` тарифа (ограничен сверху 100), веса
  алгоритма (`MATCH_WEIGHTS`) не трогаются.
- `requestsService.listOpen` — Free не видит заявку первые
  `earlyAccessDelayMin` минут после создания; для этого нужен `telegramId`
  мастера (фронтенд передаёт его в `GET /requests/open?...&telegramId=`,
  см. `loadProviderInbox` в `App.jsx`) — без него определить тариф
  запрашивающего мастера негде, фильтр просто не применяется.

`LeadCharge`/`Transaction` — `providerId`/`requestId` намеренно `SetNull`,
не `Cascade`: это финансовая/аналитическая история, обязана пережить
настоящее удаление мастера или заявки через админку (в отличие от
`Review`/`Offer` и т.п., которые каскадно удаляются вместе с мастером —
см. раздел «Удаление мастера/клиента» выше).

## Поддержка

Кнопка «Написать в поддержку» в личных кабинетах (клиент и мастер) — форма
(тема опционально + обязательный текст), не чат: односторонняя связь
пользователь → админ, ответа внутри приложения не предусмотрено. Модуль
`src/modules/support/`:
- `POST /api/support` — создаёт `SupportTicket` (номер обращения — просто
  `id`, как «Заявка №N» у заявок) и асинхронно, не блокируя ответ, зовёт
  `notifyAdmin()` из `src/bot/bot.ts` — тот же Telegram-бот уведомлений
  шлёт админу личным сообщением отформатированную карточку обращения (от
  кого, из какой роли, тема, текст). Отправка в Telegram обёрнута в
  `.catch()` — если бот недоступен или `SUPPORT_ADMIN_CHAT_ID` не задан,
  обращение всё равно остаётся в БД, просто без уведомления (см. `WARN` в
  логах).
- `GET /api/admin/support` — список всех обращений для вкладки
  «Поддержка» в админке (просмотр, без статусов/переписки).

`notifyAdmin(text)` в `bot.ts` — переиспользуемый канал уведомлений админу
в личку, не только для поддержки. Требует `SUPPORT_ADMIN_CHAT_ID` (числовой
Telegram chat id, не @username — Bot API не умеет слать по имени
пользователя без диалога с ним). Получить свой id — написать что угодно
`@userinfobot`.

## Чат по заказу (клиент ↔ мастер)

В отличие от «Поддержки» выше — это настоящая двусторонняя переписка,
модуль `src/modules/messages/` (`Message` в схеме, привязана к `Request`).

- **Доступность**: тред открывается только когда у заявки есть отклик со
  `status: "accepted"` (заказ подтверждён клиентом) — до этого момента чата
  не существует, `getThreadContext()` в `service.ts` кидает 409. Участник
  со стороны мастера — не отдельное поле, а провайдер именно этого
  принятого отклика; если отклик позже поменяется, история чата всё равно
  привязана к `Request`, а не к конкретному `Offer`.
- `GET /api/messages/thread/:requestId?telegramId=` — отдаёт сообщения и
  сразу помечает прочитанными все непрочитанные от собеседника (открытие
  треда = прочтение, отдельного `markRead` нет).
- `POST /api/messages/thread/:requestId` — `{ telegramId, text }`,
  роль (client/provider) определяется по `telegramId` так же, как в
  `getThreadContext`. Синхронно только пишет `Message` в БД; уведомление
  собеседнику в Telegram (`notifyUser` из `bot.ts`) — не `await`, в `.catch`,
  тот же паттерн, что и `notifyAdmin`/`notifyUser` в остальных модулях, и
  гейтится флагом получателя `notifyChat` (`User`/`Provider`).
- `GET /api/messages/unread-count?telegramId=&role=client|provider` —
  агрегат для бейджей на фронте (дашборд, списки заказов). `role`
  обязателен: один и тот же `telegramId` может быть и клиентом, и
  мастером одновременно (см. «Роли пользователей» выше), считать
  непрочитанные нужно только в контексте текущего кабинета.
- **Realtime — поллинг, не WebSocket.** Осознанный выбор ради простоты
  (MVP, один инстанс на VPS, без доп. инфраструктуры): фронтенд опрашивает
  `GET .../thread/:id` раз в 4с, пока открыт экран чата. Если когда-нибудь
  понадобится WebSocket — это отдельный заход с изменением Dockerfile/
  деплоя, не делать по умолчанию.

## Известные грабли (уже наступили и исправлены — не повторять)

1. **Alpine + Prisma требует OpenSSL явно.** `node:22-alpine` не
   содержит OpenSSL из коробки — движок Prisma падает с невнятной
   ошибкой парсинга JSON. Решение уже в `Dockerfile`:
   `RUN apk add --no-cache openssl libssl3` в обоих стейджах сборки, +
   `binaryTargets = ["native", "linux-musl-openssl-3.0.x"]` в
   `schema.prisma`. Не убирать.
2. **Миграций не существует, используем `prisma db push`.** Файлы
   миграций (`prisma/migrations/`) никогда не генерировались через
   `prisma migrate dev` — только `schema.prisma` руками. Поэтому
   `Dockerfile` запускает `prisma db push --accept-data-loss`, а не
   `migrate deploy` (которая ничего не создаст без файлов миграций).
   Если в будущем перейдём на нормальные миграции — нужно сначала
   сгенерировать историю миграций из текущей живой БД, иначе
   `db push` при следующем деплое может расхождение схемы бесшумно
   исправить не так, как ожидается.
3. **`DATABASE_URL` в Coolify — поле "Postgres URL (internal)"** на
   карточке PostgreSQL-ресурса, работает только для связи между
   контейнерами внутри одного сервера Coolify (не наружу).
4. **Удаление отзыва не пересчитывало рейтинг.** `reviewsService.remove`/
   `clientReviewsService.remove` изначально просто делали `prisma.review
   .delete()`, не трогая `Provider.rating`/`User.rating` — рейтинг
   «зависал» на старом значении (например, 5.0 с 1 отзывом при удалённом
   через админку единственном отзыве, хотя список отзывов уже пустой).
   Тот же баг был и в каскадном удалении заявки (`requestsService.remove`
   удаляет вместе с ней её отзывы, но раньше тоже не пересчитывал).
   Теперь оба места и `requestsService.remove` используют общие
   `recomputeProviderRating`/`recomputeUserRating` из `src/db/ratings.ts`
   — **любое** новое место, которое удаляет `Review`/`ClientReview`
   (напрямую или каскадом), обязано вызвать соответствующую функцию
   пересчёта после удаления. Для починки уже накопленного на проде
   расхождения (до этого фикса) есть разовая кнопка «Пересчитать все
   рейтинги» в админке (раздел «Отзывы») — `POST
   /api/admin/recompute-ratings`, пересчитывает всех мастеров и клиентов
   с нуля по фактическим отзывам.

## Деплой

- Сервер: VPS (Hetzner-класса), Ubuntu 24.04, управляется через Coolify.
- Домен backend: `api.goservices.lol` (фронтенд — `goservices.lol`,
  отдельный ресурс/репозиторий).
- Build Pack в Coolify: `Dockerfile`.
- env переменные задаются в Coolify (Environment Variables), не в `.env`
  на проде — `.env` только для локальной разработки, см. `.env.example`.
- После пуша в `main` деплой в Coolify запускается вручную кнопкой
  Deploy (автодеплой по пушу не настроен).

## Локальный запуск

```bash
npm install
cp .env.example .env   # заполнить DATABASE_URL локальной/тестовой БД
npx prisma db push
npm run seed            # начальные категории: Ремонт/Уборка/Перевозки
npm run dev
```

Админка: `http://localhost:3000/admin`.

## Связанный репозиторий

Фронтенд (React + Vite, Telegram Mini App) — отдельный репозиторий
`board_miniapp`, деплоится отдельным ресурсом в том же проекте Coolify.
Обращается к этому backend по `https://api.goservices.lol/api`.
При изменении публичных эндпоинтов здесь — проверять, не сломался ли
контракт с фронтендом (особенно формы запросов `POST /requests`,
`POST /providers/register`, `POST /offers`).

## Правила при работе над этим репо

- Не переписывать модульную структуру ради «универсальности» — простота
  и явные модули важнее.
- Не добавлять фреймворки/библиотеки без необходимости (проект
  сознательно минималистичный на MVP-стадии).
- Все пользовательские тексты (ошибки, лейблы) — на русском, это
  продукт для русскоязычных пользователей в Тбилиси.
- Перед изменением `Dockerfile` или `schema.prisma` — перечитать раздел
  «Известные грабли» выше.

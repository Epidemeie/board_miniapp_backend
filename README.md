...
`/api/admin/users/:id` (GET) отдаёт объединённую карточку пользователя — клиентская
история (заявки/отзывы) плюс вложенный `provider` целиком (услуги/районы/отклики/
отзывы), если этот человек ещё и мастер; админка использует её на вкладке
«Пользователи» (два подтаба — Клиент/Мастер, один запрос на оба). `/api/admin/providers/:id`
(GET) — то же самое, но только карточка мастера. PUT у обоих — редактирование
(у мастера — включая набор услуг, районов и `blocked`; у клиента — имя/username/`blocked`),
DELETE удаляет мастера/клиента каскадом (услуги, районы, отклики, отзывы, заявки —
см. schema.prisma). `blocked` — модерация админом, ограничивает только новые
действия пользователя, см. «Блокировка админом» в CLAUDE-backend.md.
